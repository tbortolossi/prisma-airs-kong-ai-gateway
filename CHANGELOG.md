...
- **`docs/coverage.md` separates the two provider questions.** Upstream LLM
  provider: every one Kong proxies, with no list to maintain here, because
  `ai-custom-guardrail` runs after `ai-proxy` has normalised the exchange and
  never sees a provider's native wire format. Client-facing `formats` on the AI
  Model: that one does matter, because the scanned text is rebuilt from
  `messages[]` in the caller's body. On `openai`, the default for every
  provider, turn attribution and `params.tool_scan` work fully; on a native
