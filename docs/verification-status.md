...
| The widened removal (`request.querystring` wiped outright; `request.uri`, `request.url` and `upstream_uri` rebuilt from the Kong PDK to drop the query string; `ai.proxy.tried_targets` wiped) covers all five fields, and none of it reaches Kong's own proxy access log on the same stdout | **LAB-VERIFIED 2026-09-15** | Closed — `POST /v1/chat/completions?apikey=PROBE&other=kept` against the live data plane produced a serializer record with zero occurrences of the marker, no `custom_fields_by_lua` error in the node log, and both processing-latency fields still populated (549 / 365 ms) |
