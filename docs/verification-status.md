...
| `airs_contents` assembles and attributes array-shaped `content` (`type: "text"` parts joined and labelled `user:` / `assistant:`), and falls back to the full flat `text_source` text on any part shape it does not recognise | Offline-tested only — 14 assertions, `scripts/test-verdict-functions.lua` (125 total) | One Open WebUI request with an image attached, checking the echo payload carries the attributed text rather than the flat fallback |
| The `airs-error-sanitizer` policy also rewrites the `HTTP 500` produced by a guardrail function that raises or fails to render ("failed to evaluate function", "failed to render by function"), not only the guardrail-call-failure body already verified | **SYNTHESIZED** | Reproducing a raising or mis-templated guardrail function against the sanitizer, on both control plane shapes |
| The `airs-diagnostics-log` policy also removes `proxy-authorization` from the recorded headers | **SYNTHESIZED** | A request carrying it, checked against the captured record |
| A query-string credential (`?apikey=...`) reaches the diagnostics-log record in five fields, not one: `request.querystring.apikey`, `request.uri`, `request.url`, `upstream_uri` and `ai.proxy.tried_targets[0].upstream_uri` | **LAB-VERIFIED 2026-09-15**, against the live data plane | Closed — this is what drove the widened removal below |
| The widened removal (`request.querystring` wiped outright; `request.uri`, `request.url` and `upstream_uri` rebuilt from the Kong PDK to drop the query string; `ai.proxy.tried_targets` wiped) covers all five fields, and none of it reaches Kong's own proxy access log on the same stdout | **SYNTHESIZED**, not independently re-run | A request carrying `?apikey=...`, checked against the captured record and against the node's access log line |
