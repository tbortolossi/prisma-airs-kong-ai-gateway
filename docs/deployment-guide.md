...
**Forward proxy.** If your data planes reach the internet through an HTTP forward proxy, `config.proxy_config` exists for that: host, port and scheme for HTTP and HTTPS, optional proxy credentials, and a `no_proxy` list. It ships commented out for the same reason as `rejection_mode`, above. The `http_proxy_host` / `http_proxy_port` pair is verified: both the `INPUT` and `OUTPUT` scan calls went through the forward proxy and the request completed normally. Prisma AIRS itself is reached over HTTPS, so a customer behind an egress proxy sets the `https_proxy_host` / `https_proxy_port` pair instead. That pair has not been exercised here.
