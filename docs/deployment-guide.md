...
**Observability.** Blocked requests are visible in three places: the Kong data plane logs, the AI Gateway metrics, and the Prisma AIRS scan logs in Strata Cloud Manager. The client-facing message and the internal record are deliberately different: the client sees only "Blocked by Prisma AIRS" and, when present, the `scan_id`; the category and the detection names are recorded in `config.metrics.block_reason` and `config.metrics.block_detail` in Kong's own telemetry, and in the Prisma AIRS scan log itself. `scan_id` is the correlation key across all three.
...
| Second guardrail instance rejected at apply time | Two `ai-custom-guardrail` instances declared on the same Service or the same Route | Kong keys a plugin instance uniquely per scope. Attach exactly one of `airs-scan` / `airs-prompt-scan` per scope |
